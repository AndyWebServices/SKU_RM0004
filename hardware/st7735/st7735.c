...
void lcd_display_ip(bool force_redraw) {
    static char iPSource[20] = {0};
    if (IP_SWITCH == IP_DISPLAY_OPEN) {
        char *newIp = get_ip_address_new();
        if (strcmp(iPSource, newIp) != 0 || force_redraw) {
            // New IP address detected!
            strcpy(iPSource, newIp);

            // Blank screen, draw underline
        ...
        // Send the IP address to the lower machine
    } else {
...
void lcd_display_disk(void) {
    ...
    memTotal = diskMemSize;
    useMemTotal = diskUseMemSize;
